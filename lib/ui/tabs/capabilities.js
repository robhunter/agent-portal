...
function capSection(title, cards) {
  if (!cards || cards.length === 0) return '';
  return '<div class="status-section"><h2>' + title + '</h2>' + cards.join('') + '</div>';
}
...
async function loadCapabilities() {
  ...
  try {
    ...
    var banner = '';
    if (capCycleRunning && agentInstr.length > 0) {
      banner = '<div style="background:#fff8e1;border:1px solid #ffe082;border-radius:8px;padding:10px 14px;margin-bottom:12px;font-size:13px;color:#7a5c00">'
        + 'A cycle is running \\u2014 editing is disabled until it finishes. Saved changes take effect at the next wake.'
        + '</div>';
    }

    var agentCards = agentInstr.map(function(i) {
      return capExpandableCard({
        iconBg: '#e8eaf6',
        icon: '\\u{1F4D8}',
        title: i.label,
        subtitle: i.source,
    ...
    if (agentCards.length > 0) {
      html += '<div class="status-section"><h2>Instructions</h2>' + banner + agentCards.join('') + '</div>';
    }

    html += capSection('Shared Core', sharedInstr.map(function(i) {
      return capExpandableCard({
        iconBg: '#eceff1',
        icon: '\\u{1F310}',
        title: i.label,
    ...
    html += capSection('MCP Servers', (data.mcpServers || []).map(function(srv) {
      return '<div class="status-card" style="display:flex;align-items:center;gap:12px">'
    ...
    html += capSection('Scripts &amp; Tools', (data.scripts || []).map(function(sc) {
      return '<div class="status-card" style="display:flex;align-items:center;gap:12px">'
        ...
        + '<div style="flex:1;min-width:0">'
        + '<div style="font-weight:600;font-size:14px;font-family:monospace">' + escapeHtml(sc.name) + '</div>'
        + (sc.description ? '<div style="font-size:13px;color:#555">' + escapeHtml(sc.description) + '</div>' : '')
        + '</div>'
        + '<span style="font-size:11px;color:#888">' + (sc.size < 1024 ? sc.size + 'B' : Math.round(sc.size / 1024) + 'KB') + '</span>'
        + '</div>';
    }));

    html += capSection('Skills', (data.skills || []).map(function(sk) {
      return capExpandableCard({
        iconBg: '#f3e5f5',
        icon: '\\u{1F4A1}',
    ...
    html += capSection('Managed Workspaces', (data.workspaces || []).map(function(ws) {
      return '<div class="status-card" style="display:flex;align-items:center;gap:12px">'
    ...
    html += '</div>';
    contentEl.innerHTML = html === '<div style="max-width:800px"></div>'
      ? '<div class="empty">Nothing discovered for this agent.</div>'
      : html;
  } catch (err) {
