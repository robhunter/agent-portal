...
${authorCSS}
  ...
  .tts-ctrl:hover { color: #1a73e8; }
  .tts-playpause-btn { font-size: 18px; }
  .tts-time { font-size: 11px; color: #888; font-variant-numeric: tabular-nums; min-width: 36px; text-align: center; }
  .tts-scrub { flex: 1; min-width: 80px; height: 4px; accent-color: #1a73e8; cursor: pointer; }
  .tts-speed { font-size: 11px; padding: 2px 4px; border: 1px solid #ddd; border-radius: 4px; background: #fff; color: #555; cursor: pointer; }
  @media (max-width: 768px) {
    .tts-player { flex-wrap: wrap; gap: 2px; padding: 4px 6px; }
    .tts-scrub { min-width: 60px; order: 10; width: 100%; }
    .tts-time { font-size: 10px; }
  }
  ...
  /* Media library */
  .library-filter-bar { display: flex; gap: 8px; flex-wrap: wrap; margin-bottom: 16px; align-items: center; }
  .library-filter-bar select, .library-filter-bar input { padding: 6px 10px; border: 1px solid #ddd; border-radius: 4px; font-size: 13px; background: #fff; font-family: inherit; }
  .library-filter-bar input { flex: 1; min-width: 140px; }
  .media-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); gap: 14px; }
  .media-card { background: #fff; border-radius: 8px; border: 1px solid #e8e8e8; overflow: hidden; cursor: pointer; display: flex; flex-direction: column; transition: box-shadow 0.15s; }
  .media-card:hover { box-shadow: 0 2px 8px rgba(0,0,0,0.1); }
  .media-card-image { width: 100%; aspect-ratio: 2/3; object-fit: cover; background: #f5f5f5; display: block; }
  .media-card-placeholder { width: 100%; aspect-ratio: 2/3; background: #f0f0f0; display: flex; align-items: center; justify-content: center; font-size: 48px; color: #bbb; }
  .media-card-body { padding: 10px; flex: 1; display: flex; flex-direction: column; }
  .media-card-title { font-size: 13px; font-weight: 600; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; margin-bottom: 6px; color: #222; }
  ...
  .media-detail-info { min-width: 0; }
  .youtube-embed { position: relative; padding-bottom: 56.25%; height: 0; overflow: hidden; margin: 12px 0; border-radius: 8px; }
  .youtube-embed iframe { position: absolute; top: 0; left: 0; width: 100%; height: 100%; border: none; }
  .library-feedback-panel { margin-top: 16px; padding-top: 16px; border-top: 1px solid #eee; }
  @media (max-width: 768px) {
