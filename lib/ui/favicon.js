...
//
// Five states, because the data supports five and collapsing them would lie:
//
//   running    filled green disc     a cycle holds the lock right now
//   scheduled  clock                 cron is installed, enabled, AND the daemon
//                                    is alive, so it WILL fire — just not this
//                                    second
...
//                                    never coming.
//   degraded   amber triangle        cron is live but the last cycle ended in an
//                                    error. Runs are still coming; they are not
//                                    landing. A stopwatch here would read as
//                                    healthy, which is the whole problem.
//   idle       grey ring             cron is absent, or installed and switched
//                                    off — the agent is off on purpose. Hollow,
//                                    so "off" reads as absence.
//
...
const FAVICONS = {
  ...
  // Filled triangle rather than another ring or disc: at 16px the silhouette is
  // the only thing that survives, and #d29922 next to the stopwatch's #b26a00 is
  // not a difference anyone can name in a tab strip. The colour is Flenderson's
  // `.status-dot.partial`, so "degraded" is the same yellow in both UIs.
  degraded:
    `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">` +
    `<path d="M16 3 L30 28 L2 28 Z" fill="#d29922"/></svg>`,
 ...
 * running), "off on purpose" wins over every complaint about a schedule nobody
 * asked to keep, and a dead daemon beats "scheduled" so the promise is never
 * made. Errors come last because they are the softest signal here — the next
 * cycle may well land.
 */
function faviconState(status) {
  ...
  // A commented-out wake line leaves the file installed and the daemon alive, so
  // `installed` alone would show a stopwatch for an agent that is switched off.
  // Only an explicit false counts: a payload from an older portal has no
  // `enabled` field, and must not turn every one of those agents grey.
  if (!cron.installed || cron.enabled === false) return 'idle';
  if (!cron.daemonRunning) return 'broken';
  return status.lastCycleErrored ? 'degraded' : 'scheduled';
}
