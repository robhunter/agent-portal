...
// In projects-sidebar mode, url-state.js owns history (it calls pushURLState after
// switchTab). When true, switchTab skips its own history write so that override is
// the single source of truth — otherwise each tab switch pushes two entries (#259).
let skipTabHistory = false;
...
async function updateHarnessStatus() {
  ...
  if (!el) return;
  var label = (PORTAL_CONFIG && PORTAL_CONFIG.harnessLabel) || 'Claude';
  try {
    const res = await fetch('/api/harness/status');
    const data = await res.json();
    if (data.harnesses) {
      el.innerHTML = data.harnesses.map(h =>
        '<span class="status-dot ' + (h.loggedIn ? 'status-green' : 'status-red') + '" style="width:8px;height:8px;vertical-align:middle;margin-right:4px"></span>'
        + escapeHtml(h.type) + (h.loggedIn ? ': authenticated' : ': not authenticated')).join(' &middot; ');
      el.title = data.loggedIn ? '' : 'a harness credential is missing or expired';
      el.style.color = data.loggedIn ? '#2e7d32' : '#c62828';
    } else if (data.loggedIn) {
      const sub = data.subscriptionType ? ' (' + escapeHtml(data.subscriptionType) + ')' : '';
      el.innerHTML = '<span class="status-dot status-green" style="width:8px;height:8px;vertical-align:middle;margin-right:4px"></span>' + escapeHtml(label) + ': authenticated' + sub;
      el.title = (data.email || '') + (data.authMethod ? ' via ' + data.authMethod : '');
      el.style.color = '#2e7d32';
    } else {
      el.innerHTML = '<span class="status-dot status-red" style="width:8px;height:8px;vertical-align:middle;margin-right:4px"></span>' + escapeHtml(label) + ': not authenticated';
      el.title = label + ' credentials missing or expired';
...
function switchTab(tab) {
  ...
  if (skipTabHistory) {
    // projects-sidebar mode: url-state.js owns history via pushURLState — skip here.
  } else if (suppressPushState) {
...
// Edit handlers key off an entry's stable timestamp, not its array position.
// Infinite-scroll prepend (_loadOlderEntriesAuto) shifts every existing entry's
// index, so a positional index baked into an onclick at render time goes stale and
// would target the wrong entry after older entries load (#259 bug 1). The key is a
// sanitized ts (alphanumerics only) so it embeds safely in an onclick attribute and
// in element ids; the resolved entry's raw .ts is what the save PUT sends.
function _journalEntryKey(ts) { return String(ts == null ? '' : ts).replace(/[^0-9A-Za-z]/g, ''); }
function _findJournalEntryByKey(key) {
  if (!_journalEntries) return -1;
  for (var i = 0; i < _journalEntries.length; i++) {
    if (_journalEntries[i] && _journalEntryKey(_journalEntries[i].ts) === key) return i;
  }
  return -1;
}
...
function renderJournalEntry(e, idx) {
  ...
  const editBtn = e.author === 'rob'
    ? '<button class="edit-entry-btn" onclick="startEditEntry(\\\'' + _journalEntryKey(e.ts) + '\\\', false)" title="Edit">Edit</button>' : '';
...
function startEditEntry(key, isProjectJournal) {
  var idx = _findJournalEntryByKey(key);
  if (idx < 0) return;
  ...
  var tagSelect = '<select id="edit-tag-' + key + '">'
  ...
  bodyEl.innerHTML = '<div class="edit-entry-form">'
    + '<textarea id="edit-text-' + key + '">' + escapeHtml(entry.content) + '</textarea>'
    + '<div class="form-row">'
    + tagSelect
    + '<button onclick="saveEditEntry(\\\'' + key + '\\\', ' + isProjectJournal + ')">Save</button>'
    + '<button class="cancel-btn" onclick="cancelEditEntry(\\\'' + key + '\\\', ' + isProjectJournal + ')">Cancel</button>'
    + '</div>'
    + '</div>';
}

async function saveEditEntry(key, isProjectJournal) {
  var idx = _findJournalEntryByKey(key);
  if (idx < 0) return;
  var entry = _journalEntries[idx];
  if (!entry) return;
  var text = document.getElementById('edit-text-' + key).value.trim();
  var tag = document.getElementById('edit-tag-' + key).value;
  if (!text) return;
...
async function cancelEditEntry(key, isProjectJournal) {
  if (isProjectJournal) {
    if (typeof loadProjectJournal === 'function') await loadProjectJournal();
    else await loadJournal();
  } else {
...
async function loadStatus() {
  ...
  try {
    ...
    // Harness credentials
    var hLabel = (PORTAL_CONFIG && PORTAL_CONFIG.harnessLabel) || 'Claude';
    html += '<div id="harness-status-detail" class="status-card">'
      + '<div class="label">' + escapeHtml(hLabel) + ' Credentials</div>'
    ...
    // Load harness credentials detail
    try {
      const harnessRes = await fetch('/api/harness/status');
      const harnessData = await harnessRes.json();
      const detailEl = document.getElementById('harness-status-detail');
      if (detailEl) {
        if (harnessData.harnesses) {
          detailEl.querySelector('.value').innerHTML = harnessData.harnesses.map(h =>
            escapeHtml(h.type) + ': <span style="color:' + (h.loggedIn ? '#2e7d32' : '#c62828') + '">'
            + (h.loggedIn ? 'Authenticated' : 'Not authenticated') + '</span>').join(' &middot; ');
        } else if (harnessData.loggedIn) {
          detailEl.querySelector('.value').innerHTML = '<span style="color:#2e7d32">Authenticated</span>'
            + (harnessData.email ? ' &middot; ' + escapeHtml(harnessData.email) : '')
            + (harnessData.subscriptionType ? ' &middot; ' + escapeHtml(harnessData.subscriptionType) : '')
            + (harnessData.authMethod ? ' &middot; ' + escapeHtml(harnessData.authMethod) : '');
        } else {
