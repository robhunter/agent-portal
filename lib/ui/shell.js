...
function buildHTML(config) {
  ...
  await Promise.all([loadProjects(), loadNextRun(), updateStatusDot(), updateHarnessStatus(), loadBadges()]);
...
window.addEventListener('popstate', function(e) {
  ...
  const portalConfig = {
    ...
    sidebarType,
    hasTTS: !!process.env.GOOGLE_TTS_API_KEY,
    harnessLabel: (config.harness && config.harness.type) ? config.harness.type.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase()).replace(/ Code$/, '') : 'Claude',
  };
...
setInterval(updateHarnessStatus, 60000);
