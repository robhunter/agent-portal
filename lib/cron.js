...
// Toggling cron off comments the wake line out, and nothing stops that line
// from being indented. Anchor the comment test after the leading whitespace so
// an indented "#" still reads as disabled rather than as a live schedule.
const isWakeLine = (line) => line.includes('wake.sh');
const isDisabledWakeLine = (line) => isWakeLine(line) && line.trimStart().startsWith('#');
const isActiveWakeLine = (line) => isWakeLine(line) && !line.trimStart().startsWith('#');
...
function getNextRun(cronFile) {
  ...
  const wakeLine = cronContent.split('\n').find(isActiveWakeLine);
  if (!wakeLine) {
    const commented = cronContent.split('\n').find(isDisabledWakeLine);
    if (commented) {
      const commentedExpr = commented.replace(/^\s*#\s*/, '').trim().split(/\s+/).slice(0, 5).join(' ');
